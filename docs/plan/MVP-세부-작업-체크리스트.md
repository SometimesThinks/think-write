...
- [x] `OPENAI_API_KEY` 로드 구현
- [x] `OPENAI_MODEL` 선택값 처리 또는 기본값 `gpt-5-nano` 고정
- [x] 서버 전용 환경변수 접근 지점 정리

### 2-2. 오류 처리 연결

- [x] 환경변수 누락 시 `PROVIDER_NOT_CONFIGURED` 반환
- [x] 잘못된 key 인증 실패 시 `PROVIDER_AUTH_FAILED` 매핑
...
- [x] OpenAI 요청 payload 구성
- [x] 기본 모델을 `gpt-5-nano`로 연결
- [x] autocomplete 용도에 맞는 최소 prompt/context 전략 정리
- [x] provider 응답을 공통 응답 형태로 매핑

### 3-2. 오류 처리

- [x] 인증 실패 매핑
- [x] rate limit / provider 오류 매핑
- [x] 예상하지 못한 응답 포맷 방어 처리
...
- [x] `app/` 아래에 client editor 컴포넌트 추가
- [x] Tiptap 기본 에디터 인스턴스 생성
- [x] 현재 정적 editor shell 영역을 실제 editor로 교체
- [x] editor placeholder 또는 초기 텍스트 정책 정리

### 4-2. 텍스트 중심 편집 환경 정리

- [x] 불필요한 리치 텍스트 기능 제거 또는 미노출 처리
- [x] 문단 입력 중심 레이아웃 구성
- [x] editor 스타일을 현재 페이지 톤과 맞게 최소 적용

### 4-3. Ghost 연동용 editor 이벤트 표면 마련

- [x] 현재 문단 텍스트 읽기 방식 정리
- [x] 커서 위치 읽기 방식 정리
- [ ] `Tab` / `Esc` / 추가 입력 / 커서 이동 이벤트 감지 지점 정리
- [x] 이후 Ghost Complete 훅이 붙을 수 있는 인터페이스 초안 마련
...
- [x] language settings 로컬 상태 구조 정의
- [x] 새로고침 후 복원 방식 연결
- [x] editor/completion 요청 코드에서 설정값 접근 가능하게 연결
